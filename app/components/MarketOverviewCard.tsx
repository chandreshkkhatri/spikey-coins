...
export default function MarketOverviewCard({
  ...
  return (
    <Link
      href={href}
      className="rounded-2xl border border-border bg-surface p-6 transition-colors hover:border-gold/30"
    >
      <p className="text-sm font-medium text-zinc-400">{pair}</p>
      <p className="mt-1 text-lg font-semibold text-white">{name}</p>
      <p className="mt-3 font-mono text-2xl font-bold text-gold">
        ${formattedPrice}
      </p>
