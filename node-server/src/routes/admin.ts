...
export async function getAllUsers(req: Request, res: Response): Promise<void> {
  try {
    ...
    logger.info(
      `Admin: User list accessed by ${req.user?.username} - ${users.length} users found`
    );
    res.json({
      success: true,
      data: users,
...
export async function clearOldData(req: Request, res: Response): Promise<void> {
  try {
    ...
    logger.info(
      `Admin: Data cleanup performed by ${req.user?.username} - ${result.deletedCount} documents removed from ${collection}`
    );
    res.json({
      success: true,
 ...
 */
export async function runBinanceCoinGeckoMatcher(
  req: Request,
  res: Response
): Promise<void> {
  try {
    ...
    logger.info(
      `Admin: Binance-CoinGecko matcher started by ${req.user?.username}`
    );
    ...
    logger.info(
      `Admin: Binance-CoinGecko matcher completed - Matched: ${result.matched}, Unmatched: ${result.unmatched}`
    );
 ...
 */
export async function getBinanceCoinGeckoMatcherStatus(
  req: Request,
  res: Response
): Promise<void> {
  try {
...
export async function summarizeArticle(
  ...
  try {
    ...
    logger.info(
      `Admin: Article summarization started by ${req.user?.username} for URL: ${url}`
    );
    ...
    logger.info(
      `Admin: Article summarization completed by ${req.user?.username} - Title: ${summary.title}`
    );
 ...
 */
export async function updateSummaryPublication(
  req: Request,
  res: Response
): Promise<void> {
  try {
    ...
    logger.info(
      `Admin: Summary publication status updated by ${req.user?.username} - ID: ${summaryId}, Published: ${isPublished}`
    );
...
export async function getAllSummaries(
  ...
  try {
    ...
    const publishedCount = await summariesCollection.countDocuments({
      isPublished: true,
    });
    const unpublishedCount = totalCount - publishedCount;

    logger.info(
      `Admin: Summaries list accessed by ${req.user?.username} - ${summaries.length} summaries returned`
    );
